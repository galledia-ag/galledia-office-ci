"""
fill_kurzbrief.py — befuellt die Galledia Kurzbrief-Vorlage (.dotx) und speichert als .docx.
...
TEMPLATE_PATH = SKILL_DIR / "templates" / "Kurzbrief-Vorlage Galledia.dotx"

# Standard-Notes (10 vordefinierte Optionen aus der Kurzbrief-Vorlage).
# Note9 in der Original-Vorlage ist "per Fax an Nummer:" — das nutzt Galledia
# nicht mehr (Markenhandbuch v1.5). Wir ersetzen den Default durch
# "per E-Mail an:" — Anwender koennen via 'notes' jedes Label ueberschreiben.
DEFAULT_NOTES = {
...
def _patch_officeatwork_part(raw: bytes, data: dict) -> bytes:
    ...
    # Notes (Kurzbrief-spezifisch): User-Override > Default
    user_notes = data.get("notes", {}) or {}
    for key, default_text in DEFAULT_NOTES.items():
        updates[key] = user_notes.get(key, default_text)
...
def fill(data: dict, template: Path, output: Path) -> dict:
    ...
    # --- Notes (Note1..Note10 als Content Controls) ---
    user_notes = data.get("notes", {}) or {}
    for key, default_text in DEFAULT_NOTES.items():
        report["tags_filled"][key] = apply_to_all(key, user_notes.get(key, default_text))
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Galledia Kurzbrief Generator")
